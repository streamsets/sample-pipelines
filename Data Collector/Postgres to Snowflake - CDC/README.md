...
<h1><p align="center">Postgres to Snowflake - CDC</p></h1>

# Postgres to Snowflake - CDC (Updated: 2022.05)
...
The first pipeline (**Postgres_Snowflake[a]_BULK**) will be a batch pipeline that runs once and reads data from Postgres and loads data to Snowflake.  This is in a separate folder located [here](https://github.com/streamsets/sample-pipelines/tree/master/Data%20Collector/Postgres%20to%20Snowflake%20-%20BULK)

The second pipeline (**Postgres_Snowflake[b]_CDC**) will be a streaming pipeline that will replicate changes from your source to the destination performing inserts, updates and deletes.
...
![Pipeline](images/PostgrestoSnowflake_pipeline.png "Postgres CDC to Snowflake")

## DOCUMENTATION

[PostgreSQL CDC Client](https://docs.streamsets.com/portal/platform-datacollector/5.0.x/datacollector/UserGuide/Origins/PostgreSQL.html#concept_cfs_4m4_n2b)
...
[Click Here](./Postgres_Snowflake[b]_CDC.zip?raw=true) to download the pipeline and save it to your drive.
...
![Step 5](images/PostgrestoSnowflake_step5.png "View the results")
