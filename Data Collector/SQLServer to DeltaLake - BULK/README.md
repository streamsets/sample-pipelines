...
<h1><p align="center">SQLServer to DeltaLake - BULK</p></h1>

# SQLServer to DeltaLake - BULK (Updated: 2022.05)
...
This pipeline (**SQLServer_DeltaLake[a]_BULK**) will be a batch pipeline that runs once and reads data from SQLServer and loads data to DeltaLake.

Once the BULK load has been done, you can then set up the second pipeline (**SQLServer_DeltaLake[b]_CDC**) which will be a streaming pipeline that will replicate changes from your source to the destination performing inserts, updates and deletes.  This is in a separate folder locate [here](https://github.com/streamsets/sample-pipelines/tree/master/Data%20Collector/SQLServer%20to%20DeltaLake%20-%20CDC)
...
![Pipeline](images/SQLServertoDeltaLake_pipeline.png "SQLServer BULK to DeltaLake")
...
[Click Here](./SQLServer_DeltaLake[a]_BULK.zip?raw=true) to download the pipeline and save it to your drive.
